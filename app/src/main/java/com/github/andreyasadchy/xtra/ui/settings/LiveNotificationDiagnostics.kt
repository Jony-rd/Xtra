...
internal fun liveNotificationDiagnostics(context: Context): String = buildString {
    ...
    appendLine("Twitch web token present: ${auth.gqlTokenWebPresent}")
    appendLine("Twitch web cookie header present: ${auth.gqlWebCookieHeaderPresent}")
    appendLine("Twitch user ID present: ${auth.userIdPresent}")
    appendLine("Twitch token scopes present: ${auth.scopesPresent}")
